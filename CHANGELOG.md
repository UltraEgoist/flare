...
### Added
- **Source Map V3 対応**: コンパイル時に `.js.map` ファイルを自動生成。ブラウザDevToolsで `.flare` ソースを直接デバッグ可能。VLQ エンコーディングを外部依存なしで実装
- **VSCode 定義ジャンプ**: `Go to Definition` (Ctrl+Click/F12) を実装。`{{ 式 }}`、`@event`、`:bind`、`:attr` 内のシンボルから宣言へジャンプ
- **セキュリティ再監査**: 全コードベースの包括的監査を実施し、3件の新規脆弱性を修正

### Security
- **S-16**: `txSafe()` を書き直し、ネストテンプレートリテラルを正しく処理
- **S-19**: `scopeCss()` の CSS セレクタインジェクション防止（tagName エスケープ）
- **S-22**: dev サーバーの symlink TOCTOU 修正（realpathSync + lstatSync）
- **NEW-V8**: ポート番号の範囲バリデーション追加（1〜65535）
- **NEW-V10**: ホバー表示のJSDocマークダウンインジェクション防止
- **NEW-V12**: VSCode イベントハンドラ正規表現のReDoS防止（修飾子上限10）

### Fixed
- **VSCode fn ハンドラ警告**: フォールバック検出を追加（script 生テキストから fn 宣言を直接検索）
- **VSCode キャッシュ**: activate() でハッシュキャッシュをクリアし、拡張更新後の再解析を保証
